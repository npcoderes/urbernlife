...
const showHomeCareProducts = () => {
    ...
    if (homeCareProducts.length === 0) {
        cartElement.innerHTML = '<p>No products found in the Home-Care category.</p>';
        return;
    }

    // Render each grocery product
    homeCareProducts.forEach((curProd) => {
        const { id, name, category, image, price, actualprice, stock, description } = curProd;

        console.log(homeCareProducts);
        // Clone the template
